...
import {
  LayoutDashboard, FileText, MessageSquare, Settings, LogOut, BookOpen, Menu, X, Layers
} from 'lucide-react';
  ...
  const Sidebar = () => (
    <aside className="w-64 flex flex-col h-full" style={{ background: '#ffffff', borderRight: '1px solid #ecf0f0' }}>
      ...
      <div className="flex items-center gap-3 px-6 py-5" style={{ borderBottom: '1px solid #ecf0f0' }}>
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shadow-md" style={{ background: 'linear-gradient(135deg, #1a598a, #015599)' }}>
      ...
      <div className="px-3 py-4" style={{ borderTop: '1px solid #ecf0f0' }}>
        <div className="flex items-center gap-3 px-3 py-3 mb-2 rounded-xl" style={{ background: '#f8fafb' }}>
          <div className="w-9 h-9 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-sm" style={{ background: 'linear-gradient(135deg, #1a598a, #015599)' }}>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: '#f8fafb' }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Mobile topbar */}
        <header className="flex items-center justify-between px-5 py-3.5 lg:hidden shadow-sm" style={{ background: '#ffffff', borderBottom: '1px solid #ecf0f0' }}>
          <button onClick={() => setSidebarOpen(true)} style={{ color: '#67787a' }} className="hover:text-gray-900 transition-colors">
            <Menu size={22} />
          </button>
          <p className="font-bold text-sm" style={{ color: '#0c1e21' }}>Blog Admin</p>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: 'linear-gradient(135deg, #1a598a, #015599)' }}>
